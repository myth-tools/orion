...

{
	...
	echo ""
	echo '```sh'
	echo "# Via Go (any platform)"
	echo "go install ${GO_MODULE}/cmd/orion@${NEW_TAG}"
	echo ""
	echo "# ── Linux ────────────────────────────────────────────────────────"
	echo "# Pre-built archive"
	...
	echo "# ── macOS ────────────────────────────────────────────────────────"
	echo "# Pre-built archive"
	echo "tar xzf ${BIN}_${ver}_darwin_arm64.tar.gz"
	echo "sudo install ${BIN} /usr/local/bin/"
	echo ""
	echo "# Raw binary"
	echo "chmod +x ${BIN}_${ver}_darwin_arm64"
	echo "sudo mv ${BIN}_${ver}_darwin_arm64 /usr/local/bin/${BIN}"
	echo ""
	echo "# ── Windows ──────────────────────────────────────────────────────"
	echo "# PowerShell"
	echo "Expand-Archive -Path ${BIN}_${ver}_windows_amd64.zip -DestinationPath ."
	echo "Move-Item .\\${BIN}.exe C:\\Windows\\System32\\${BIN}.exe"
	echo ""
	echo "# Command Prompt"
	echo "tar -xf ${BIN}_${ver}_windows_amd64.zip"
	echo "move ${BIN}.exe C:\\Windows\\System32\\${BIN}.exe"
	echo ""
	echo "# ── Termux / Android ─────────────────────────────────────────────"
	echo "# Pre-built archive"
	echo "tar xzf ${BIN}_${ver}_termux_arm64.tar.gz"
	echo "install ${BIN} ~/../usr/bin/"
	echo ""
	echo "# Raw binary"
	echo "chmod +x ${BIN}_${ver}_termux_arm64"
	echo "mv ${BIN}_${ver}_termux_arm64 ~/../usr/bin/${BIN}"
	echo ""
