...
func TestVersionConsistency_MainGoVersionMatchesMetadata(t *testing.T) {
	t.Parallel()
	ver := wantVersion(t)
	if version != "dev" && version != ver {
		t.Errorf(
			"cmd/orion/main.go version is %q — expected %q (from metadata.yaml) or \"dev\"",
			version, ver,
		)
	}
}
